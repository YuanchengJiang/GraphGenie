...
if __name__ == "__main__":
    ...
    test = Neo4jTesting()
    print("\033[91m##### neo4j 12988 #####\033[0m")
    base_query = "OPTIONAL MATCH p=shortestPath((a:User)-[:IN_GENRE*]-(c:Genre)) RETURN DISTINCT p"
    mutated_query = ""
    test.testing(base_query, mutated_query)
    print("\033[91m##### neo4j 12991 #####\033[0m")
    base_query = "OPTIONAL MATCH (s1)-[s0:DIRECTED]-()<-[s2:ACTED_IN]-(s1) RETURN count(s1);"
    mutated_query = "OPTIONAL MATCH (s1)-[s0]-()<-[s2:ACTED_IN]-(s1) WHERE s0:DIRECTED RETURN count(s1);"
    test.testing(base_query, mutated_query)
    print("\033[91m##### neo4j 12996 #####\033[0m")
    base_query = "MATCH (a)-[b:RATED]->(c) RETURN count(a) SKIP 1;"
    mutated_query = "MATCH (a)-[b:RATED]->(c) RETURN count(b) SKIP 1;"
    test.testing(base_query, mutated_query)
